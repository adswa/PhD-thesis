...
However, studies decoding working memory content found decodable patterns to be non-stationary.
...
In this method, different classifiers are trained on different times in training data, and tested on all available times in the test data, thus revealing whether the neural code is stable or dynamically evolving.
...
This framework proposes working memory maintenance via increased residual calcium levels at the presynaptic terminals of neurons, which causes a short-term synaptic facilitation, akin to synaptic weights that link neurons coding for working memory item.
...
Hidden Markov models (HMMs) became a common method to model ensemble activity as a sequence of \textit{neural states} \citep{rainer2000neural}.
These distinct neural states were often hypothezised to reflect steps in mental processing \citep[e.g.,][]{seidemann1996simultaneously}.
