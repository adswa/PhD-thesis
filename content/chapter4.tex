...
Stimulus combinations were selected to fulfill the following requirements:
\begin{enumerate}
	...
	\item In the first option, all magnitude and probability options are equally often shown
...
In approximately half of the trials both magnitude and probability were either equal or greater in one of the two options compared to the other, requiring no integration of stimulus properties to assess which options' value is higher.
In roughly 10\% of trials, called ``no-brainer'' in all following analyses, both magnitude and probability were strictly greater in one of the two options, posing the easiest decision form.
...
Based on the Maxwell equations, \gls{SSS} decomposes \gls{meg} signals ($\phi$) into elementary magnetic fields from sources within the sensor helmet (the \textit{internal} subspace) and into an orthogonal set for fields arising from sources outside (the \textit{external} subspace)  \citep{taulu2006spatiotemporal}. \todo{(TODO: Make the math nice, link to dimensionality reduction from intro?)}
...
\begin{figure}
	...
	\caption[Power spectral density before and after ZAPLine filtering]{Power spectral density of all MEG channels
		...
		Two spikes at 50Hz (power-line frequency) and 60Hz (likely an artifact of the stimulus presentation) are markedly reduced afterwards. \todo{include filter properties}
	}
...
On average, participants gained 349.5 points in the experiment (min=300.5, max=393; top boxplot in Figure \ref{fig:gains}).
To assess whether this exceeds chance level performance, I conducted a simulation of 10.000 experiments with random choice behavior (bottom boxplot in Figure \ref{fig:gains}), confirming that all participants performed significantly better than chance at the 95\% confidence level (dotted line in Figure \ref{fig:gains}).
A different measure of performance is reaction time.
The average reaction time pooled across participants was 0.93s.
Contrary to the expectation that no-brainer trials would pose easier decisions, the difference in average reaction times for no-brainer trials compared to standard trials was negligent (0.91s for no-brainer compared to 0.94s for standard trials; Figure \ref{fig:reactiontimes}).
...
The stimulus properties relevant for the eventual behavioral choice in each trial were stripe frequency (reward magnitude), stripe orientation (reward probability), and their combination (expected value).
...
The second mean to assess which stimulus properties influenced choice was a k-fold cross-validated logistic regression analysis.

The Spearman rank correlation between experiment performance (total gain) and model accuracy was $r=0.53$ (p=0.012)
...
\begin{figure}
	...
	\end{subfigure}
		...
		\caption[Behavioral results]{\ref{fig:gains} Points gained in the experiment across subjects. \ref{fig:avgreactions} Average reaction times over trials for all subjects. \ref{fig:reactiontimes} Reaction times across trials and subjects for standard trials (orange) and trials with unambigous. \ref{fig:logregbehavior} Relative influence of stimulus properties on choice (left option)}
		\label{fig:behav}
	\end{figure}
