...
One may be that the shared response model, taking multi-subject or multi-trial data into account, identifies subspaces that are not relevant to the targets' decoding.
...
Alternatively, working memory maintenance in decision making may have strong idiosyncratic components, and shared signals derived from several individuals do not capture their participant-specific neural codes well.
Likewise, the interpretations of the identified components may not be as straight-forward as initially envisioned.
The visualization approaches in my analyses for example focused on finding simple amplitude-based differences between stimulus types in the shared space.
