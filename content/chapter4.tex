...
It was acquired as part of the SFB 779 project B16N on offline value representations in sequential decision making at the Otto-von-Guericke University Magdeburg in 2016 by \citet{kaiser} and has not been previously published.
...
Following a metal test, MEG data were acquired on an Elekta Neuromag TRIUX System with 306 sensors (204 planar gradiometers and 102 magnetometers).
...
\gls{SSS} and \gls{tSSS} remove strong interference by external noise sources and sources within the body itself from the MEG signal.
...
These are estimated based on the fact that their strength typically exceeds that of sensor noise and thus, unlike brain signal, leaks into both the internal and external part of the \gls{SSS} reconstruction.
After detecting components with a high temporal correlation between the external and internal parts, \gls{tSSS} removes close-by artifacts by projecting the components common to the internal and external subspace out of the internal subspace.
...
ZAPline filtering \citep{de2020zapline} was performed to remove them using meegkit \citep{barascud2022}.
Figures TODO and TODO show power spectral density plots of the signal pre and post (effective window size: 2.048 s) \gls{tSSS}.

Before splitting the continuous data into epochs,
