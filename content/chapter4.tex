...
Contrary to \citet{murray2017stable, machens2010functional}, the upcoming analyses do not confine the neural signal to a specific brain area such as the PFC, but attempt to find neural signatures of decision-relevant working memory items across the cortex.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.6\textwidth]{memento/analysis_setup.png}
...
The analysis set of the \textit{memento} study consisted of a variety of methods.
...
And unlike \citet{machens2010functional}, the resulting components, although they clearly captured trial-related information, were not easily interpretable.
There are multiple possible reasons for this.
One may be that the shared response model, taking multi-subject or multi-trial data into account, identifies subspaces that are not relevant to the chosen targets' decoding.
In other words, the lower-dimensional space they identified may not have been the ``right'' one.
For example, different neural processes may have been more prominent and thus over-represented, especially since whole-brain data was the input of all analyses, and thus a variety of other cognitive processes were likely reflected in the neural signal.
Both \citet{machens2010functional} and \citet{murray2017stable} had confined their analyses to prefrontal areas, and used electro-physiological recordings which are spatially much more precise than the overlaid neural signals in sensor-space \gls{meg} data.
Alternatively, working memory maintenance in decision making may have strong idiosyncratic components or inter-trial variability, and shared signals derived from several individuals or trials do not capture their participant- or trial-specific neural codes well.
Likewise, the interpretations of the identified components may not be as straight-forward as initially envisioned.
The visualization approaches in my temporal decoding analyses for example focused on finding simple amplitude-based differences between stimulus types in the shared space.
This approach would have failed to find more complex representations, for example based on interactions between several components, would they exist.
An alternative reason may lie in different analysis approaches. In \citet{murray2017stable}, mnemonic subspaces were constructed from averaged delay activity over stimulus conditions and time.
...
Another reason may be that coding of decision-relevant information in working memory is indeed dynamic and activity silent.
However, the observed decoding accuracy during the first half of the delay is clearly not activity silent.
And the results of \citet{kaiserposter} showed that above-chance decoding of stimulus characteristics vanishes even in temporal generalization analyses, yet a decision without such information would surely be challenging.
Therefore, a plausible reason why stimulus features could not be decoded through the entire delay may be that it is not stimulus characteristics that are encoded but an alternative representation, such as decision preparation.\\
The behavioral analyses shed light on possible decision making strategies participants employed.
In conjunction with the behavioral analyses, the temporal generalization results provide evidence that participants employed strategies based on sequential evaluation of magnitude and probability in decision making, and, importantly, that a neural representation of this strategy persists in the delay period that is not based on stimulus features:
...
Above this, a decision-related neural code was better represented during the maintenance phase than absolute stimulus property values, falling in line with findings by \citet{hunt2013trial} on a working memory representation in a neural space of actions.
This result would support a decision-preparation in working memory rather than stimulus features.
This latter interpretation, however, is complicated by the fact that the experiment design was explicitly focused on uncovering stimulus feature representations, and arguably inept to test this hypothesis thoroughly.
...
Given the possibility that other neural processes may create more prominent signals, or that important idiosyncratic processes could be discarded as noise, future studies could attempt to identify these dominant signals, and subtract them from individual participants' neural signal do remove dominating sources of uninteresting shared signal or investigate idiosyncratic signals in particular.
