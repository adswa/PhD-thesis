...
But despite the wealth of theories and methodological approaches, the field has not yet converged at a full understanding of working memory.
...
All analyses were set up as portable DataLad datasets.
...
\begin{figure}[H]
	...
	\caption[Memento analysis outline]{Project outline as linked DataLad datasets for provenance capture, disk usage reduction, and computational efficiency. A \gls{BIDS}-structured raw dataset is the anchor of all analyses. A common preprocessing pipeline is the basis of three major \gls{meg} analyses. If raw or preprocessed data changes, analysis results can be recomputed automatically.}
...
The data were acquired as part of the SFB 779 project B16N (``offline value representations in sequential decision making'') at the Otto-von-Guericke University Magdeburg by \citet{kaiser}, and have not been previously published.
Ethics approval was obtained from the institutional review board of the medical faculty of the University Magdeburg under application number 101/15.
...
Each trial (Figure \ref{fig:memento_trial}) started with a fixation cross (1000-1900ms, jittered), followed by the first stimulus on the left side of the screen (700ms), a 2000ms delay period through which a central ``or'' was presented, the second stimulus option on the right side (700ms), and a feedback screen.
...
Underlying software packages are listed in Table \ref{tab:software}, and the software environment required for all analyses is also available as a Singularity image that can be found alongside the source code.
\begin{center}
\begin{table}[H]
	\begin{tabular}{ l l l }
		\hline
...
\begin{figure}[H]
	...
	\caption[Preprocessing overview]{Overview of preprocessing steps and their details.}
	\label{fig:preproc}
\end{figure}
...
The filter properties are reported in Figure \ref{fig:preproc} and a visualization is depicted in Figure \ref{fig:filter}.
...
This analysis mirrored the previous approach, but epochs were not split by stimulus type and pooled instead, and the correlation distance was not calculated between time series, but between $k$ features per time point.
...
In the upcoming analyses, however, I used them to express signal as a function of frequency rather than time, and used this transformation from a time-resolved representation of data to a representation in the frequency domain to find shared signals with different temporal signatures.
...
For each sensor with a signal, a random weight is drawn that determines the strength with which the signal is scaled.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.35\textwidth}
		\includegraphics[width=\textwidth]{memento/simulation/model-weights_versus_ground-truth_all-ds.jpg}
		\caption{SRM on time-resolved data}
		\label{fig:sim-timeresolved-weights}
	\end{subfigure}
	\begin{subfigure}{0.35\textwidth}
		\includegraphics[width=\textwidth]{memento/simulation/model-weights_versus_ground-truth_all-ds_spectral.jpg}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\begin{subfigure}{0.33\textwidth}
		\includegraphics[width=\textwidth]{memento/spectral_srm/avg-signal_shared-shape_spectral-srm_5-feat_per-comp_response-locked.jpg}
...
For example, trials in which the first stimulus option had different probability values were contrasted with each other.
...
\begin{figure}
	...
	\includegraphics[width=\textwidth]{memento/spectral_srm/trialtype-magnitude_avg-signal_shared-shape_spectral-srm_5-feat_per-comp_delay.jpg}
	\caption{Delay (split by stimulus probability)}
	\label{fig:spectral-comp-delay-splitprob}
\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{memento/spectral_srm/trialtype-probability_avg-signal_shared-shape_spectral-srm_5-feat_per-comp_delay.jpg}
		\caption{Delay (split by stimulus magnitude)}
		\label{fig:spectral-comp-delay-splitmag}
	\end{subfigure}
\hfill
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{memento/spectral_srm/event-choice_avg-signal_shared-shape_spectral-srm_5-feat_per-comp_delay.jpg}
		\caption{First stimulus presentation (split by choice)}
		\label{fig:spectral-comp-delay-splitchoice}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{memento/spectral_srm/avg-signal_shared-shape_spectral-srm_5-feat_per-comp_response-locked_leftvsright.jpg}
...
\begin{center}
	\begin{table}
		\begin{tabular}{p{.15\textwidth} p{.4\textwidth} p{.25\textwidth} p{.2\textwidth}}
			...
			Sliding window 		& Sliding windows integrate neural signals over time or space, and are commonly used to improve the signal to noise ratio as well \citep{hari2017primer}. This parameter control which form of sliding window was used. & temporal sliding window, spatiotemporal sliding window & temporal sliding window \\
			Trial averaging 	& To improve the signal to noise ratio, it is common to average trials of the same trial type \citep{hari2017primer}. & None, 5, 10 & 10 (None in spectral SRM) \\
		...
		\end{tabular}
		\caption[Overview of pipeline parameters]{Overview of configurable pipeline parameters and the values explored in a grid search.}
...
Both types of sliding windows were used with a window size of 10 samples, and are illustrated in Figures \ref{fig:temporal-slider} and \ref{fig:spatiotemporal-slider}.\\
...
For this, I computed confusion matrices in 100ms time slices to visualize dominantly decoded labels across the trial structure (Figure \ref{fig:decoding-probability-conf}).
