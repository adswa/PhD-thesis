...
\begin{figure}
	...
	\caption[Neural trajectories]{An illustration of neural trajectories through state-space for two decision alternatives (blue) and two response alternatives (red), for two tasks in a delayed-response-mapping paradigm.  Adapted from Jocham, Krauel, Hanke (unpublished)}
...
Liquid helium makes the flux transformers superconductive by cooling them down to -269°C.
...
\begin{figure}
	\centering
	\includegraphics[width=0.4\textwidth]{memento/preprocessing_overview.png}
...
To this end, it transforms the $N=306$-dimensional signal vector into a lower dimensional subspace that spans all measurable signals, the ``SSS basis'' $S$. Its subspaces $S_{in}$ and $S_{out}$ contain the biomagnetic signal and arbitrary external interference, respectively.
...
\texttt{autoreject}, an algorithm to reject and repair bad trials as well as to estimate bad sensors on a per-trial basis \citep{jas2017autoreject}, was then used on the first 200 of these epochs to estimate the noise level and compute rejection thresholds.
...
In these analyses, the goal is to predict a discrete target $y$ from a data matrix $X$ with $n$ observations, for example, if a participant performs a left or right response (target) based on the stimulus properties (features) in each trial.
In scikit-learn's terms, an \textit{estimator} is what performs this prediction of a target variable $y$ based on input data $X$.
A \textit{scorer} is a method that evaluates an estimators predictions on a given dataset.
...
For each analysis, I ran a permutation test with $N=10000$ repetitions with shuffled targets to identify clusters of significant decoding accuracy.
...
The signal of interest can thus clearly be expressed in fewer than 306 dimensions.
In the following analyses, I therefore sought to find out whether I can find meaningful low-dimensional structure in the high-dimensional dataset.
The method I investigated was \gls{SRM} \citep{NIPS2015_b3967a0e}.
\gls{SRM} is a functional alignment method that [ADD MATH HERE].
...
\gls{SRM} is successfully and widely used with fMRI data \citep{bazeille2021empirical}, in particular for naturalistic acquisitions with free-viewing tasks and movie stimuli (cite Sam), the method is only rarely used in \gls{meg} or trial-based experimental designs.
...
the exact timing or duration of such shared activity is subject to intra- and inter-individual variation.
, and thus not necessarily phase-locked. [CITE]
